        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                if self.steps % self.logging_freq == 0 and update:
                    ...
                    #a=self.model.fusion.weights[0].item()
                    #b = self.model.fusion.weights[1].item()
                    #log_out += "Fusion Weights => a: {:.4f}, b: {:.4f} || ".format(a, b)
...
if __name__ == "__main__":
    ...
    train(cfg_file=args.config)#!/usr/bin/env python
import torch
...
def train(cfg_file: str) -> None:
    ...
    train_data, dev_data, test_data, gls_vocab, txt_vocab = load_data( # 这个函数是来自 data中的； 这里我将sign向量手动处理了
        data_cfg=cfg["data"] #读取数据 得到train, dev, test gls txt的内容
    )

    # build model and load parameters into it
    do_recognition = cfg["training"].get("recognition_loss_weight", 1.0) > 0.0 # 本身值是1.0
    do_translation = cfg["training"].get("translation_loss_weight", 1.0) > 0.0 # 本身值是1.0
    model = build_model( # 建立模型的参数设置
        cfg=cfg["model"],
        gls_vocab=gls_vocab,
        txt_vocab=txt_vocab,
        sgn_dim=sum(cfg["data"]["feature_size"])
