...
import torch
import torch.nn.functional as F
from signjoey.loss import SignCLLoss


class FeatureFusion(nn.Module):
    def __init__(self, dim=1024):
        super().__init__()
        self.weights = nn.Parameter(torch.tensor([0.5, 0.5]))  # 可学习权重

    def forward(self, cnn_feat, i3d_feat):
        """
        自动插值 I3D 特征并融合
        ...
        :return: [B, T1, D]
        """
        B, T1, D = cnn_feat.shape
        i3d_up = F.interpolate(i3d_feat.transpose(1, 2), size=T1, mode='linear', align_corners=True).transpose(1, 2)
        w = torch.softmax(self.weights, dim=0)  # 保证 a + b = 1
        return w[0] * cnn_feat + w[1] * i3d_up
...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = FeatureFusion()  # ✅ 添加这一行
        ...
        self.signcl_loss = SignCLLoss()
        self.signcl_weight = 0.2  # 或从外部 config 读取更好
    ...
    def forward(
        self,
        sgn: Tensor,
        sgn_mask: Tensor, # [batch_size, 1, max_seq_len]
        sgn_lengths: Tensor,
        txt_input: Tensor,
        ...
        if self.do_recognition: # 如果做识别过程 得到gloss的过程
            # Gloss Recognition Part
            # N x T x C
            gloss_scores = self.gloss_output_layer(encoder_output) # 一个线性层：Linear(hidden_dim, gloss_vocab_size)
            # N x T x C
            gloss_probabilities = gloss_scores.log_softmax(2)
            # Turn it into T x N x C
            gloss_probabilities = gloss_probabilities.permute(1, 0, 2) # 形状调整
        else:
    ...
    def encode(
        self, sgn: dict, sgn_mask: Tensor, sgn_length: Tensor
    ) -> (Tensor, Tensor):
        """
        ...
        #embed_input = self.sgn_embed(sgn, sgn_mask)
        # embed_input = self.sgn_embed(sgn)  # ✅ 只有 sgn
        embed_input = self.sgn_embed(sgn, sgn_mask)  # ✅ CNN 用法

        return self.encoder(
            #embed_src=embed_input,# ✅ I3D 只有 sgn
            embed_src=self.sgn_embed(x=sgn, mask=sgn_mask),# ✅ CNN 用法
    ...
    def get_loss_for_batch( # 计算CTC loss - 识别和 翻译的loss crossEntropy loss
        self,
        batch: Batch,
...
def build_model(
    ...
    if cfg.get("use_i3d", False): # 这个不会在执行了
        input_dim = cfg.get("model", {}).get("feature_dim", 1024)
        hidden_dim = cfg["encoder"]["hidden_size"]  # 要映射到的维度
        sgn_embed = nn.Linear(input_dim, hidden_dim)
        sgn_embed.embedding_dim = hidden_dim  # 👈 添加这行手动赋值
    else: # 直接走CNN的路
        sgn_embed = SpatialEmbeddings(
...
""" 
本来在decode上面的 
    def encode(self, sgn: dict, sgn_mask: Tensor, sgn_length: Tensor) -> (Tensor, Tensor):
        
        Encodes the source sign input with CNN + I3D fusion.

        :param sgn: dict，包括 {"cnn": Tensor, "i3d": Tensor}
        :param sgn_mask: 掩码
        :param sgn_length: 有效长度
        :return: encoder 特征 + hidden
        
        cnn_feat = sgn["cnn"]  # [B, T1, D]
        i3d_feat = sgn["i3d"]  # [B, T2, D]

        fused_feat = self.fusion(cnn_feat, i3d_feat)  # 插值+加权融合 → [B, T1, D]self.txt_vocab

        embed_input = self.sgn_embed(x=fused_feat, mask=sgn_mask)  # 位置编码
        return self.encoder(embed_src=embed_input, src_length=sgn_length, mask=sgn_mask)
