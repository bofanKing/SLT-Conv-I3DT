...
def test(
    ...
    if output_path is not None:
        ...
        # ✅ 输出一个文件包含：视频名 | 真实文本 | 预测文本
        detailed_output_path = output_path + ".detailed_test_output_a0.6.txt"
        with open(detailed_output_path, "w", encoding="utf-8") as f_out:
                for name, ref, hyp in zip(test_data.sequence,
                                        test_best_result["txt_ref"],
                                        test_best_result["txt_hyp"]):
                    f_out.write(f"{name} | REF: {ref} \n HYP: {hyp}\n")
