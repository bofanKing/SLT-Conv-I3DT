...
def build_vocab_from_datasets(examples):
    gloss_counter = Counter()
    text_counter = Counter()

    for ex in examples:
        gloss_counter.update(ex["gls"].split())
        text_counter.update(ex["txt"].split())

    gloss_vocab = Vocab(gloss_counter, specials=["<pad>", "<unk>", "<s>", "</s>"])
    text_vocab = Vocab(text_counter, specials=["<pad>", "<unk>", "<s>", "</s>"])
