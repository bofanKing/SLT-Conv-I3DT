        ...
        if frame_subsampling_ratio:
            ...
            for idx, (features, length) in enumerate(zip(self.sgn, self.sgn_lengths)):
                ...
                if random_frame_subsampling and is_train:
                    init_frame = random.randint(0, (frame_subsampling_ratio - 1)) # 随机起始帧 - 对于训练阶段
                else:
                    init_frame = math.floor((frame_subsampling_ratio - 1) / 2) # 固定起始帧 测试结果一样

                tmp_data = features[: length.long(), :] # 截取有效帧
                tmp_data = tmp_data[init_frame::frame_subsampling_ratio] # 以 ratio 为步长，间隔采样
                tmp_sgn[idx, 0 : tmp_data.shape[0]] = tmp_data
                tmp_sgn_lengths[idx] = tmp_data.shape[0]

            self.sgn = tmp_sgn[:, : tmp_sgn_lengths.max().long(), :] # 更新batch内容
            self.sgn_lengths = tmp_sgn_lengths # 获取样本帧长度

        if random_frame_masking_ratio and is_train: # 掩码阶段
            tmp_sgn = torch.zeros_like(self.sgn)
            num_mask_frames = (
                (self.sgn_lengths * random_frame_masking_ratio).floor().long()
            )
            for idx, features in enumerate(self.sgn): # 随机掩码部分内容
                features = features.clone()
                mask_frame_idx = np.random.permutation(
                    int(self.sgn_lengths[idx].long().numpy())
                )[: num_mask_frames[idx]]
                features[mask_frame_idx, :] = 1e-8 # 用0进行替换 去掉有效特征 类似于 dropout效果
                tmp_sgn[idx] = features # 放回batch
            self.sgn = tmp_sgn

        self.sgn_dim = sgn_dim
        self.sgn_mask = (self.sgn != torch.zeros(sgn_dim))[..., 0].unsqueeze(1) #对于pad 无效内容进行舍弃
